#!/bin/bash
bag=${PWD##*/}-groundtruth-cartographer.bag
if [ ! -f odom_ground_truth_cartographer.bag ] || [ "$1" = "--clean" ]; then
    echo "Converting ground truth odometry!"
    rosrun rawseeds_ros gtextodom2bag.py --frame_id odom --topic /odom *GROUNDTRUTH-CARTOGRAPHER.csv odom_ground_truth_cartographer.bag
fi
...
rosrun rawseeds_ros merge_bags.py -i odom_ground_truth_cartographer.bag imu.bag sick_rear.bag sick_front.bag hokuyo_front.bag hokuyo_rear.bag -o $bag
